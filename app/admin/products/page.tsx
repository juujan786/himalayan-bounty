...
const categories = ['Shilajit', 'Nuts', 'Dry Fruits', 'Seeds', 'Other'];

export default function AdminProductsPage() {
  const { products, createProduct, deactivateProduct, updateProduct } = useData();
  const [showModal, setShowModal] = useState(false);
  const [form, setForm] = useState({ name: '', category: '', price: '', oldPrice: '', description: '', images: [] as string[], featured: false, weight: '', weightUnit: 'g' });
  const [imageFiles, setImageFiles] = useState<File[]>([]);
  const [editId, setEditId] = useState<string | null>(null);
  const [editForm, setEditForm] = useState({ name: '', category: '', price: '', oldPrice: '', description: '', images: [] as string[], featured: false, weight: '', weightUnit: 'g' });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    createProduct({ ...form, weight: form.weight ? form.weight + form.weightUnit : '', active: true });
    setForm({ name: '', category: '', price: '', oldPrice: '', description: '', images: [], featured: false, weight: '', weightUnit: 'g' });
    setImageFiles([]);
    setShowModal(false);
  };
  ...
  const handleEditSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!editId) return;
    updateProduct(editId, { ...editForm, weight: editForm.weight ? editForm.weight + editForm.weightUnit : '' });
    setEditId(null);
  };
    ...
    <div>
      ...
      <div className="flex flex-col gap-4">
        {products.filter(p => p.active).map(p => (
          <div key={p.id} className="flex items-center gap-4 bg-[#f7f6f3] rounded-xl p-4">
            <div className="flex gap-2">
              {p.images && p.images.map((img, idx) => (
                <img key={idx} src={img} alt={p.name} className="w-12 h-12 rounded object-cover bg-white" />
              ))}
            </div>
            <div className="flex-1">
              <div className="font-semibold">{p.name} <span className="text-xs text-gray-500 ml-2">{p.category}  ${p.price} {p.weight && <span className="ml-2">({p.weight})</span>}</span></div>
              <div className="text-xs text-gray-500">{p.description}</div>
              {p.featured && <span className="inline-block bg-yellow-300 text-yellow-900 text-xs px-2 py-0.5 rounded ml-2">Featured</span>}
            </div>
            <div className="flex gap-3 text-gray-500">
              <button title="Edit" onClick={() => openEditModal(p)} className="text-blue-500"><FaEdit /></button>
              <button title="Deactivate" className="text-red-500" onClick={() => deactivateProduct(p.id)}><FaTrash /></button>
            </div>
          </div>
        ))}
      </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-lg p-8 w-full max-w-md relative">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-3">
              ...
              <button type="submit" className="bg-[#235c42] text-white px-4 py-2 rounded hover:bg-[#1d4d36] mt-2">Add Product</button>
            </form>
          </div>
        </div>
      )}
